 ...
 * <b>用法一：</b>
 * <p>
 * Lock lock = new RedisLock("test",true);//获取资源锁
 * </p>
 * <p>
 * lock.lock();//加锁业务等待时间30秒，如果锁住就锁60秒(不unlock的情况下)
 * </p>
 * <p>
 * lock.lock(3000);//业务等待时间3秒，如果锁住就锁60秒(不unlock的情况下)
 * </p>
 * <p>
 * lock.isLocked()//加锁后判断是否锁住了
 * </p>
 * <p>
 * lock.lockTime(20) 锁住就锁20秒，默认30秒内去获取redis资源(不unlock的情况下)
 * </p>
 * <p>
 * lock.lock(3000,10) 业务等待时间3秒，锁住就锁10秒(不unlock的情况下)
 * </p>
 * <p>
 * 业务逻辑代码
 * </p>
 ...
 * <b>用法二：</b>
 * <p>
 * Lock lock = RedisLock.getInstance("test", true);//获取资源锁，并缓存
 * </p>
 * <p>
 * lock.lock();//加锁业务等待时间30秒，如果锁住就锁60秒(不unlock的情况下)
 * </p>
...
public class RedisLock implements Lock {
	...
	/**
	 * 状态标志，=true 对应的key加锁成功 unlock后= false
	 */
	...
	public boolean isLock() {
		if (RedisClusterUtils.get(this.key) != null) {
			return Boolean.TRUE;
		}
	...
	public static RedisLock getInstance(String key, boolean isJoin) {
		if (isJoin) {
			key = String.valueOf(RedisClusterUtils.get(key));
		}
	...
	public boolean lock(long timeout, int lockTime) {
		...
		try {
			while ((System.nanoTime() - nano) < timeout) {
				if (RedisClusterUtils.setnx(key, LOCKED)) {
					if (lockTime > 0) {
						RedisClusterUtils.expire(key, lockTime);
					} else {
						RedisClusterUtils.expire(key, EXPIRE);
					}
					locked = true;
					isSucces = true;
		...
		System.out.println("isSucess="+isSucces);
		return isSucces;
	}

	/**
	 * 加锁 默认业务等待30秒，如果30秒还未获取到锁，则业务还会往下走， 最好使用isLocked()判断是否加锁成功
	 */
	@Override
	public void lock() {
		lock(TIME_OUT);
	...
	public void unlock(boolean isClear) {
		if (locked) {
			if (logger.isDebugEnabled()) {
				logger.debug("unlock RedisLock[" + key + "].");
			}
			if (isClear) {
				// map中有就删除
				map.remove(key);
